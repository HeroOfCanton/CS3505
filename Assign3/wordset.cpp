...
namespace cs3505
{
  ...
  void wordset::remove (std::string element)
  {
    ...
    {
      if (current->data == element) {  // Does a char-by-char comparison
	// If element is first item in list
	if(last == NULL) {
	...
	// If item to be removed is last item in list, set last->next to NULL
	...
	// If item is anywhere but beginning or end
	else {
  ...
  wordset & wordset::operator= (const wordset & rhs)
  {
    ...
    // Create new table to match capacity of rhs table
    this->capacity = rhs.capacity;
    this->count = 0;
    this->table = new node*[capacity];

    // Clear out memory in new table in case there was something else there
    for (int i = 0; i < capacity; i++)
      table[i] = NULL;

    // Copy data from rhs to this new table  
    for (int i = 0; i < capacity; i++) 
    {
  ...
  /** This function populates an array of strings with elements from the set.
    ...
    * copied into the array in positions [0..n-1].  
    * If 'n' is larger than the number of elements in the set, 
    * all the elements in the set are copied into the first positions 
    * of the array, and excess positions in the array remain unchanged.
    *
    * It is assumed that the array reference is valid and n is non-negative.
    */
  void wordset::get_elements(std::string *array, int n) const 
  {
    int count = 0; // keep track of how many items added, to compare to n

    // Start walking table, index by index, and copying items out until n
    for(int i = 0; i < n; i++)
    {
      node *current = table[i];  
      while(current != NULL) {
	// Important to stop when we reach n, 
	// even if n is in the middle of a list
	if(count < n ) {
